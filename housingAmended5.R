library(tidyverse)
library(reshape2)
housing = read.csv('C:/Data Science Course, Tech Academy/AssignmentP.444,MachineLearning/housing.csv')
head(housing)
...
categories = unique(housing$ocean_proximity)  # removes duplicates
cat_housing = data.frame(ocean_proximity = housing$ocean_proximity) # Split the categories off
...
train = cleaned_housing[sample, ]  # Just the samples (80% of total samples)
...
# I changed the ntree=500 in the tutorial to ntree=700 and importance=TRUE to importance=FALSE.
rf_model = randomForest(train_x, y = train_y , ntree = 700, importance = FALSE)
...
test_y = test[,'median_house_value']                   # How well does the model predict on the test data?
